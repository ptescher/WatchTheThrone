...
extension UInt16 {
    ...
    //Most Significant UInt8 (MSB)
    var highUint8: UInt8 { return UInt8( (self & 0xFF00) >> 8) }

    //Least Significant UInt8 (LSB)
    var lowUint8: UInt8 { return UInt8(self & 0x00FF) }

    var hlUint8s: [UInt8] { return [highUint8, lowUint8] }
...
extension String {
    ...
    //ok?
    var bytesWithLength: [UInt8] { return UInt16(count(utf8)).hlUint8s + utf8 }
...
class CocoaMQTTFrameConnect: CocoaMQTTFrame {
    ...
    override func pack() {
        ...
        variableHeader += client.keepAlive.hlUint8s
    }
